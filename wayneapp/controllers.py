...
class ControllerPathVariables(APIView):
    ...
    def post(self, request: Request, type: str, key: str) -> Response:
        ...
        version = self.getVersion(body)
        try:
            genericObject, created = update_or_create_generic_model(type, key, version, body["object"])
        except Exception:
            return Response({}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        #TODO should we return the saved object?
        return Response({}, status=status.HTTP_200_OK)

    def getVersion(self, body: Dict):
